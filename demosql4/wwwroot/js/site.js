    ...
    document.getElementById('TotalExperience').value=totalExp/60;
    return;
        }
...
//    $(btn).closest('tr').remove();
    ...
    var x = document.getElementsByTagName("INPUT");
        ...
        for (var cnt = 0; cnt < x.length; cnt++) {
            if (x[cnt].type == "text" && x[cnt].id.indexOf('_' + nextrowIdx + '_') > 0) { x[cnt].value = '' }
            else if (x[cnt].type == "number" && x[cnt].id.indexOf('_' + nextrowIdx + '_') > 0) { x[cnt].value = 0; };
    }
